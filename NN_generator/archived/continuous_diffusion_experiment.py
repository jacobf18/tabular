#!/usr/bin/env python3
"""
Continuous Diffusion Experiment for Staggered Adoption Patterns
"""
...
def run_continuous_diffusion_experiment():
    """Run the complete continuous diffusion experiment for staggered adoption patterns."""
    ...
    # Generate training data with staggered adoption patterns
    ...
    for i in range(5):
        ...
        axes[0, i].set_title(f'Training Mask {i+1}')
        axes[0, i].set_xlabel('Time')
        axes[0, i].set_ylabel('Units')
        
        axes[1, i].imshow(masking_matrices[i+5], cmap='RdYlBu_r', aspect='auto')
        axes[1, i].set_title(f'Training Mask {i+6}')
        axes[1, i].set_xlabel('Time')
        axes[1, i].set_ylabel('Units')
    
    plt.tight_layout()
    plt.savefig('continuous_training_masks.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    # Train the continuous diffusion model
    print("\nTraining continuous diffusion model...")
    model_path = 'continuous_diffusion_model.pth'
    ...
    for i, (rows, cols) in enumerate(test_sizes):
        ...
        for j in range(3):  # Generate 3 samples per size
            sample = sample_from_minimal_diffusion(
                model, scheduler, size_info, num_steps=25, device=device, threshold=0.5
            )
    ...
    plt.tight_layout()
    plt.savefig('continuous_diffusion_pattern_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    # Run the continuous diffusion experiment
    print("Running continuous diffusion experiment for staggered adoption patterns...")
    results = run_continuous_diffusion_experiment()
    
    print("\n" + "="*60)
    print("Continuous Diffusion Experiment Completed!")
    print("This experiment used:")
    print("- 200 training samples with staggered adoption patterns")
    print("- Complex CNN model with residual connections")
    print("- 100 epochs of training")
    print("- Continuous diffusion process (not discrete)")
    print("="*60) 