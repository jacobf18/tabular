...
def train_l_shaped_diffusion_model(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (x, size_info) in enumerate(dataloader):
            x = x.to(device)  # [B, 2, H, W] - stacked data and mask
...
def sample_l_shaped_matrices(
    ...
    with torch.no_grad():
        for i in tqdm(range(num_samples)):
            ...
            x = torch.randn(1, 2, matrix_size, matrix_size, device=device)
            ...
            # Extract mask channel and threshold
            mask = predicted[:, 1:2, :, :]  # [1, 1, H, W] - mask channel
            mask = torch.sigmoid(mask)  # Apply sigmoid to get 0-1 range
            mask = (mask > threshold).float()
            
            # Convert to numpy
            sample = mask.squeeze().cpu().numpy()
...
def main():
    ...
    model, scheduler, losses = train_l_shaped_diffusion_model(
        ...
        device=device,
        learning_rate=1e-3,
        save_path='l_shaped_diffusion_model.pth'
    )
    ...
    print(f"Model saved as: l_shaped_diffusion_model.pth")
