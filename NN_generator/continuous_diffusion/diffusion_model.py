...
class MinimalDiffusionModel(nn.Module):
    ...
    def __init__(self, max_size: int = 32):
        ...
        # Initial convolution (now only 1 input channel)
    ...
    def forward(self, x, t=None, size_info=None):
        ...
        if t is not None:
            ...
            t_emb = t_emb.unsqueeze(-1).unsqueeze(-1)  # [B, 128, 1, 1]
            t_emb = t_emb.expand(-1, -1, x.shape[2], x.shape[3])  # [B, 128, H, W]
        else:
            t_emb = torch.zeros(x.shape[0], 128, x.shape[2], x.shape[3], device=x.device)
        if size_info is not None:
            size_emb = self.size_embed(size_info)  # [B, 128]
            size_emb = size_emb.unsqueeze(-1).unsqueeze(-1)  # [B, 128, 1, 1]
            size_emb = size_emb.expand(-1, -1, x.shape[2], x.shape[3])  # [B, 128, H, W]
        else:
            size_emb = torch.zeros(x.shape[0], 128, x.shape[2], x.shape[3], device=x.device)
        # Initial convolution
...
class MinimalMatrixDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Return only the L-shaped matrix as input, and size info
        data = self.padded_data[idx]
        size_info = np.array(self.size_info[idx], dtype=np.float32)
        # Add channel dimension: [1, H, W]
        data = np.expand_dims(data, axis=0)
        return torch.tensor(data, dtype=torch.float32), torch.tensor(size_info, dtype=torch.float32)
