...
class PowerTransform(Preprocess):
        ...
        if np.any(valid_mask):
            ...
            if np.any(pos_mask):
                if lmbda == 0:
                    ...
                    result_valid[pos_mask] = (np.power(X_valid[pos_mask] + 1, lmbda) - 1) / lmbda
            ...
            if np.any(neg_mask):
                if lmbda == 2:
                    ...
                    result_valid[neg_mask] = -(np.power(-X_valid[neg_mask] + 1, 2 - lmbda) - 1) / (2 - lmbda)
        ...
        if np.any(valid_mask):
            ...
            if np.any(pos_mask):
                if lmbda == 0:
                    ...
                    result_valid[pos_mask] = np.power(lmbda * X_valid[pos_mask] + 1, 1 / lmbda) - 1
            ...
            if np.any(neg_mask):
                if lmbda == 2:
                    ...
                    result_valid[neg_mask] = 1 - np.power(-(2 - lmbda) * X_valid[neg_mask] + 1, 1 / (2 - lmbda))
    ...
    def _log_likelihood_box_cox(self, lmbda, X):
        ...
        # Log-likelihood
        n = len(X)
        log_likelihood = -n / 2 * np.log(np.var(transformed)) + (lmbda - 1) * np.sum(np.log(X))
    ...
    def fit(self, X):
        ...
        for i in range(n_features):
            ...
            if not np.any(valid_mask):
                warnings.warn(f"Column {i} contains no finite values. Setting lambda to 1.")
                self.lambdas_[i] = 1.0
                continue
            ...
            if self.method == 'box-cox':
                ...
                if np.any(column <= 0):
                    warnings.warn(f"Column {i} contains non-positive values. "
    ...
    def transform(self, X):
        ...
        if n_features != len(self.lambdas_):
            raise ValueError(f"Number of features ({n_features}) doesn't match "
        ...
        for i in range(n_features):
            if self.method == 'box-cox':
                ...
                X_transformed[:, i] = self._yeo_johnson_transform(X[:, i], self.lambdas_[i])
