...
class WandbLoggerCallback(BaseLoggerCallback):
    ...
    def __init__(self, project: str, name: str = None, config: dict = None, log_dir: str = None, id=None):
        """
        ...
        try:
            ...
            wandb.init(
                project=project,
                name=name,
                id=id if id is not None else name + datetime.now().strftime("%Y%m%d_%H%M%S"),
                config=config,
                dir=log_dir,
                resume="allow"
            )
