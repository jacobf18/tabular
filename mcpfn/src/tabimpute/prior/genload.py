...
def dense2sparse(
    ...
    assert (
        row_lengths <= num_cols
    ).all(), "row_lengths cannot exceed number of columns"
...
class LoadPriorDataset(IterableDataset):
    ...
    def _load_batch_file(self):
        ...
        if X.is_nested:
            ...
            X = sparse2dense(
                X, d.repeat_interleave(seq_lens[0].to(torch.int64)), dtype=torch.float32
            ).view(batch_size, int(seq_lens[0].item()), -1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_missing", type=int, default=10, help="Number of missing values"
    )
    parser.add_argument(
