...
class MAR_onesided_missingness(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.extreme_neighbor_mode = config.get('extreme_neighbor_mode', 'zero')  # 'zero' or 'all'
        ...
        if self.extreme_axis == 'row':
            if self.extreme_neighbor_mode == 'zero':
                ...
                self.input_size = torch.stack([
                    torch.stack([1 * (self.col_neighbor[i][t]+1) for t in range(self.T)])
                    for i in range(self.N)
                ])
            else:  # 'all'
                # All rows, normal column neighbors
                self.input_size = torch.stack([
                    torch.stack([self.N * (self.col_neighbor[i][t]+1) for t in range(self.T)])
                    for i in range(self.N)
                ])
        else:  # extreme_axis == 'column'
            if self.extreme_neighbor_mode == 'zero':
                ...
                self.input_size = torch.stack([
                    torch.stack([(self.row_neighbor[i][t]+1) * 1 for t in range(self.T)])
                    for i in range(self.N)
                ])
            else:  # 'all'
                # Normal row neighbors, all columns
                self.input_size = torch.stack([
                    torch.stack([(self.row_neighbor[i][t]+1) * self.T for t in range(self.T)])
                    for i in range(self.N)
                ])
