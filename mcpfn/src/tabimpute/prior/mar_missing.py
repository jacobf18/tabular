    ...
    def __init__(self, config):
        ...
        self.row_neighbor = torch.stack(
            [
                torch.stack(
                    [
                        torch.randint(1, config["row_neighbor_upper"] + 1, size=(1,))
                        for t in range(self.T)
                    ]
                )
                for i in range(self.N)
        ...
        self.col_neighbor = torch.stack(
            [
                torch.stack(
                    [
                        torch.randint(1, config["col_neighbor_upper"] + 1, size=(1,))
                        for t in range(self.T)
                    ]
                )
                for i in range(self.N)
        ...
        self.input_size = torch.stack(
            [
                torch.stack(
                    [
                        (self.row_neighbor[i][t] + 1) * (self.col_neighbor[i][t] + 1)
                        for t in range(self.T)
                    ]
                )
                for i in range(self.N)
    ...
    def forward(self, X):
        ...
        for i in range(self.N):
            for t in range(self.T):
                ...
                # Add self neighbor
                row_neighbor_idx = torch.cat(
                    [row_neighbor_idx, torch.tensor([i])]
                )  # Self neighbor
                col_neighbor_idx = torch.cat(
                    [col_neighbor_idx, torch.tensor([t])]
                )  # Self neighbor
