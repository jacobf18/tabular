...
class UnifiedBandit:
    ...
    def __init__(self, config):
        ...
        if self.algorithm not in valid_algorithms:
            raise ValueError(
                f"Unknown algorithm: {self.algorithm}. Use {valid_algorithms}"
            )
        ...
        if self.noise_type not in valid_noise_types:
            raise ValueError(
                f"Unknown noise_type: {self.noise_type}. Use {valid_noise_types}"
            )
    ...
    def create_arm_rewards(self, reward_matrix):
        ...
        for row in range(rows):
            ...
            if self.noise_type == "gaussian":
                ...
                raise ValueError(
                    f"Unknown noise_type: {self.noise_type}. Use 'gaussian', 'uniform', or 'student_t'"
                )
        ...
        if self.algorithm == "epsilon_greedy":
            ...
            if self.epsilon_decay is not None and time > 0:
                self.current_epsilon = max(
                    self.current_epsilon * self.epsilon_decay, 0.01
                )
            ...
            if min(arm_counts) == 0:
                ...
                if self.ucb_variant == "ucb1":
                    ...
                    confidence = self.c * np.sqrt(
                        np.log(time + 1) / arm_counts * np.minimum(0.25, variance_est)
                    )
            ...
            sampled_rewards = np.array(
                [
                    np.random.normal(
                        posterior_mean[0], np.sqrt(max(posterior_var[0], 1e-6))
                    ),
                    np.random.normal(
                        posterior_mean[1], np.sqrt(max(posterior_var[1], 1e-6))
                    ),
                ]
            )
    ...
    def _select_arm_with_pooling(self, unit, time):
        ...
        unit_experiences = [
            (t, a, r) for (u, t, a, r) in self.all_experiences if u == unit
        ]
    ...
    def _ucb_pooled(self, unit, unit_experiences, time):
        ...
        if min(combined_counts) == 0:
            ...
            if self.ucb_variant == "ucb1":
                confidence = self.c * np.sqrt(
                    (pop_variance + np.log(time + 1)) / combined_counts
                )
            elif self.ucb_variant == "ucb_tuned":
                variance_est = np.maximum(avg_rewards * (1 - avg_rewards), pop_variance)
                confidence = self.c * np.sqrt(
                    np.log(time + 1) / combined_counts * np.minimum(0.25, variance_est)
                )
    ...
    def _thompson_pooled(self, unit, unit_experiences):
        ...
        posterior_precision = (
            shrunk_prior_precision + unit_counts * self.likelihood_precision
        )

        posterior_mean = (
            shrunk_prior_precision * pop_mean + self.likelihood_precision * unit_rewards
        ) / posterior_precision
    ...
    def update_unit(self, unit, chosen_arm, observed_reward, time):
        ...
        if self.algorithm in ["epsilon_greedy", "ucb"]:
            ...
            self.unit_states["posterior_precision"][
                unit, chosen_arm
            ] += shrunk_likelihood
            ...
            for arm in range(2):
                if arm == chosen_arm:
                    self.unit_states["H"][unit, arm] += (
                        self.alpha * reward_diff * (1 - probabilities[arm])
                    )
                else:
                    self.unit_states["H"][unit, arm] -= (
                        self.alpha * reward_diff * probabilities[arm]
                    )
    ...
    def update_population(self, time):
        ...
        for arm in range(2):
            if len(arm_values[arm]) > 1:
                self.population_states["pop_variance"][arm] = max(
                    np.var(arm_values[arm]), 0.01
                )
    ...
    def get_config(self):
        ...
        if self.algorithm == "epsilon_greedy":
            ...
            config.update({"c": self.c, "ucb_variant": self.ucb_variant})
        elif self.algorithm == "thompson_sampling":
            config.update(
                {"prior_std": self.prior_std, "shrinkage_factor": self.shrinkage_factor}
            )
        elif self.algorithm == "gradient_bandit":
            config.update(
                {"alpha": self.alpha, "softmax_temperature": self.softmax_temperature}
            )
