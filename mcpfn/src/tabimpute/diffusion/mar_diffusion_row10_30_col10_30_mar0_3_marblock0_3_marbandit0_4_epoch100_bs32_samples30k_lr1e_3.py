...
class FullyConvolutionalX0Model(nn.Module):
    ...
    def forward(self, x, t, cond, cont_matrix) -> torch.Tensor:
        ...
        cond_emb = F.interpolate(
            cond_emb, size=(H, W), mode="bilinear", align_corners=False
        )  # (B, 512, H, W)
        ...
        cond_emb_global = (
            self.cond_embedding(cond).unsqueeze(-1).unsqueeze(-1)
        )  # (B, 512, 1, 1)
        ...
        global_features = (
            global_features + t_emb_512 + cond_emb_global + cont_emb_global
        )

        # === 7. CONVOLUTIONAL SELF-ATTENTION (REPLACES TRANSFORMERS) ===
        # Reshape for attention
        global_features_flat = global_features.squeeze(-1).squeeze(-1)  # (B, 512)
        global_features_attended = self.self_attention_1(
            global_features_flat.unsqueeze(1)
        )  # (B, 1, 512)
        global_features_attended = global_features_attended.squeeze(1)  # (B, 512)
        global_features_attended = global_features_attended.unsqueeze(-1).unsqueeze(
            -1
        )  # (B, 512, 1, 1)
        ...
        cond_emb_spatial_4x4 = F.interpolate(
            cond_emb_spatial_4x4, size=(4, 4), mode="bilinear", align_corners=False
        )  # (B, 512, 4, 4)
        # Create timestep embedding for spatial features (512 channels)
        t_emb_spatial_4x4 = F.interpolate(
            t_emb_512, size=(4, 4), mode="bilinear", align_corners=False
        )  # (B, 512, 4, 4)
        # Create continuous embedding for spatial features
        cont_emb_spatial_4x4 = F.interpolate(
            cont_emb_512, size=(4, 4), mode="bilinear", align_corners=False
        )  # (B, 512, 4, 4)
        spatial_features = (
            spatial_features
            + t_emb_spatial_4x4
            + cond_emb_spatial_4x4
            + cont_emb_spatial_4x4
        )

        # Spatial attention
        spatial_features_flat = spatial_features.flatten(2).transpose(
            1, 2
        )  # (B, 16, 512)
        spatial_features_attended = self.self_attention_2(
            spatial_features_flat
        )  # (B, 16, 512)
        spatial_features_attended = spatial_features_attended.transpose(
            1, 2
        ).reshape_as(
            spatial_features
        )  # (B, 512, 4, 4)
        ...
        y = F.interpolate(
            combined_features, size=original_size, mode="bilinear", align_corners=False
        )
    ...
    def sample(self, x, cond=None, cont_matrix=None):
        for t in reversed(range(1, self.n_T)):
            ...
            x = self.p_sample(
                ...
                cond,
                cont_matrix,
                torch.rand((*x.shape, self.num_classses), device=x.device),
            )
...
class MARDiffusionModel(nn.Module):
    ...
    def __init__(self, config: Dict):
        ...
        self.x0_model = FullyConvolutionalX0Model(
            n_channel=self.n_channel, N=self.N, num_classes=self.num_classes
        )
    ...
    def generate(
        ...
        if missingness_type is None:
            class_idx = torch.randint(
                0, self.num_classes, (num_samples,), device=device
            )
        else:
            class_mapping = {"bandit": 0, "mar": 1, "block_mar": 2}
            class_idx = torch.tensor(
                [class_mapping[missingness_type]] * num_samples, device=device
            )

        # Start with random discrete tokens (integer for D3PM)
        x = torch.randint(
            0, self.N, (num_samples, 1, height, width), device=device, dtype=torch.long
        )
